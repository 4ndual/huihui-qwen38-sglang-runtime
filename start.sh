...
VOLUME_ROOT='/runpod-volume/qwen38'
TARGET_MODEL_DIR="$VOLUME_ROOT/models/target"
DRAFT_MODEL_DIR="$VOLUME_ROOT/models/dflash"
...
PORT='30000'
PORT_HEALTH='8001'
HEALTH_READY_FILE='/tmp/runpod-lb-ready'
...
health_pid=''
terminate() {
  if [ -n "$sglang_pipeline_pid" ]; then kill "$sglang_pipeline_pid" 2>/dev/null || true; fi
...
test -s "$VOLUME_ROOT/manifests/sglang-live.sentinel"
